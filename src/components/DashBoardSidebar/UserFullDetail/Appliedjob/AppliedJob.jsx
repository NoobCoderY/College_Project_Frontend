...
export default function AppliedJob({ width, user }) {
  ...
  useEffect(() => {
    if (user?.userName) {
      axios
        .get(
          `${API_HOST}/bidings/viewUserBidingForAdmin?pageSize=9&pageNumber=${page}&userName=${
            user?.userName
  ...
  return (
    <div
      style={{ height: "calc(100vh - 15vw)", paddingTop: "0vw" ,paddingLeft:"0.5vw" }}
    ...
    >
      ...
      >
        <div className="serachjobbox">
          ...
          <input
            type="text"
            placeholder={"search Applied Proposals"}
      ...
      <div>
        <div className="catalogcontainerdashbaord">
          {allusers?.length > 0 ? (
            ...
            >
              There are no Biddings similar to your search .{" "}
            </div>
          )}
        </div>
        {totalpages !== 1 ? (
