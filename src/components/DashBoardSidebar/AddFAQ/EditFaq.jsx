...
export default function EditFaq() {
  ...
  const handlecategory = () => {
    if (!name || !ans) {
      ...
      axios
        .post(`${API_HOST}/faq/editFAQ`, formdata, {
          headers: {
  ...
  useEffect(() => {
    axios.get(`${API_HOST}/faq/viewFAQ?faqId=${id}`).then((res) => {
      setName(res?.data?.success?.data?.question);
      setAns(res?.data?.success?.data?.answer);
    });
  }, [id]);
